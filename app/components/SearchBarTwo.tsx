...
export default function SearchBar() {
  ...
  const handleChange = async (e) => {
    ...
    fetch(`/api/searchv2?searchQuery=${e.target.value}`)
        .then((res) => res.json())
        .then((data) => {
            console.log(data)
          setEmbedList(data);
    ...
    <div>
      ...
      <div className={styles.tweetDiv}>
        {embedList.map((s, index) => (
            <iframe scrolling="no" className="tweet" key={index} src={`https://platform.twitter.com/embed/Tweet.html?dnt=false&embedId=twitter-widget-3&frame=false&hideCard=false&hideThread=false&lang=en&theme=dark&width=550px&id=${s}`} />
        ))}
      </div>
    </div>
  );
